...
function getLangs(lang) {
    let Language = _.clone(require(`../lang/en.js`));
...
class IkaeasyNotification {
    ...
    send(notificationId, title, message, requireInteraction) {
        ...
        if (this._sended[notificationId]) {
            return;
        }

        this._sended[notificationId] = true;
...
const notification = new IkaeasyNotification();
const defOptions = require('../options.json');
const preventSent = {};
                ...
                for (let name in city.buildings) {
                    ...
                    city.buildings[name].forEach((v) => {
                        ...
                        if (this.getOptions(data.options, 'notification_building_complete_prevent')) {
                            ...
                            let key = `${data.server.domain}_${data.server.world}_${data.server.avatarId}_${city.id}_${v.building}_${v.completed}`;
                            if (preventSent[key]) {
                                return;
                            }

                            if (completed < now) {
                                preventSent[key] = true;
...
function ajax(message, callback) {
    ...
    if ((message.method) && (message.method.toLowerCase() === 'post')) {
        ...
        if (message.body) {
            options.body = $.param(message.body);
